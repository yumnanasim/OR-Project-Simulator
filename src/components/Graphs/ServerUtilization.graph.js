...
import { separateCustomerServerWise } from "../../utils/common.js";
...
const ServerUtilizationGraph = () => {
	const [serverUtilization, setServerUtilization] = React.useState([]);
	/**
	...
	const addArrivalAverageAndServiceTimeAverageAtEveryArrival = (servers) => {
  ...
  const addUtilizationAtEveryArrival = (servers) => {
    servers.forEach(server => {
